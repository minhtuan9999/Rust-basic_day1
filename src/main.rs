...
fn main() {
    ...
    let bai1 = check_array(&org_arr, &sub_arr);
    let bai2 = search_slice();
    
    println!("Bai1: {}", bai1);
    println!("Bai2: {}", bai2);

}
pub fn check_array(org_arr:&[i32],sub_arr:&[i32]) ->bool {
    let mut a = 0;
    if sub_arr.len()==0 {
        return true;
    } 
...
pub fn search_slice() -> usize {
    let mut file = File::open("bai2.txt").expect("File not found");
